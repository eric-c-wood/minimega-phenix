...
func CreateExperiment(w http.ResponseWriter, r *http.Request) {
    ...
    opts := []experiment.CreateOption{
        ...
        experiment.CreateWithAliases(req.VlanAlias.Alias,int(req.VlanAlias.Vlan)),
    }
...
// POST /experiments/{exp}/vms/{name}/memorySnap
func MemorySnapVM(w http.ResponseWriter, r *http.Request) {
    log.Debug("MemorySnapVM HTTP handler called")
    ...
    if !role.Allowed("vms/memorySnap", "create", fullName) {
    ...
    // filename to use for the in capturing a memory snapshot. If no body was provided, pass an
    // empty string to `api.<createMemorySnapshot>ToDisk` to let it create a copy based on
    ...
    if len(body) != 0 {
        ...
        if req.Filename == "" {
            log.Error("missing filename for memory snapshop")
...
// GET /topologies/{topo}/vlanAliases
func GetVlanAliases(w http.ResponseWriter, r *http.Request) {
    log.Debug("GetScenarios HTTP handler called")
    ...
    for _, t := range topologies {
        ...
        if t.Metadata.Name != topo {
            continue
        }
        
        topoSpec,err := types.DecodeTopologyFromConfig(t)
        
        if err != nil {
            log.Info("Unable to retrieve topology:%v",err) 
            continue
        ...
        for _,node := range topoSpec.Nodes() {
            ...
            for _,i := range node.Network().Interfaces() {
                if _,ok := vlans[i.VLAN()]; !ok {
                    
                    if role.Allowed("vlans", "list", i.VLAN()) {
                        vlans[i.VLAN()] = true                        
                    }   
                }                               
            }            
    ...
    for key,_ := range vlans {
        allowed = append(allowed,key) 
    }

    body, err := marshaler.Marshal(&proto.VlanAliasList{Aliases: allowed})
    if err != nil {
        http.Error(w, err.Error(), http.StatusInternalServerError)
        return
    }
